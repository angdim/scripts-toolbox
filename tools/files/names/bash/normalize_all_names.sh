...
# Предназначение: нормализира имената на файлове и директории според normalize.conf.
...
Описание:
  Комбиниран скрипт, който нормализира:
    - първо директории (от най-дълбоките)
    - после файлове
...
while IFS= read -r file; do
    ...
    changed=$((changed + 1))
    echo "FILE: $file → $parent/$new"
    log "FILE: $file → $parent/$new"

    [ "$DRYRUN" -eq 0 ] && mv "$file" "$parent/$new"
